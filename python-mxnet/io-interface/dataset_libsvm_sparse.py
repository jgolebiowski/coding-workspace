...
def write_partition(data, labels, partition_name, dataset_path):
    ...
    lab = np.array(labels)
    write_partition_sparse(dat, datashape, lab, partition_name, dataset_path)



def write_partition_sparse(data, datashape, labels, partition_name, dataset_path):
    """
    Write a new record to the dataset
...
def write_partition_dense(data, labels, partition_name, dataset_path):
    """
    Write a new record to the dataset
    ...
    sklearn.datasets.dump_svmlight_file(data.reshape((datashape[0], -1)), labels, partition, zero_based=True, comment=comment)
...
def load_partition(partition, input_path):
    ...
    data, labels = sklearn.datasets.load_svmlight_file(partition, n_features=datasize, dtype=np.float32, zero_based=True)
    data = data.toarray()
