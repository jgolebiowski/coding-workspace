...
print(correct_out - addConv2d.getValue())

# ------ Testing the gradient of conv2D
np.random.seed(231)
...
feed = mainGraph.addOperation(ga.Variable(x), doGradient=False, feederOperation=True)
filterOp = mainGraph.addOperation(ga.Variable(w), doGradient=True, feederOperation=False)
opConv2d = mainGraph.addOperation(ga.Conv2dOperation(feed, filterOp, stride=1, padding=1))
biasOpp = mainGraph.addOperation(ga.Variable(b[None, :, None, None]), doGradient=True, feederOperation=False)
addConv2d = mainGraph.addOperation(ga.AddOperation(opConv2d, biasOpp),
                                   doGradient=False, feederOperation=False)
...
addConv2d.getValue()
gaGradient = opConv2d.performGradient(input=0, out=dout)

out, cache = ga.conv_forward(x, w, b, stride=1, padding=1)
print(np.sum(abs(addConv2d.getValue() - out)))
numGradX, numGradW, numGradB = ga.conv_backward(dout, cache)

print("\ngaGrad:")
print(gaGradient.shape)
print(gaGradient)
print("\nHNgrad:")
print(numGradX.shape)
print(numGradX)
print("\ndiffGrad:")
print(gaGradient - numGradX)
