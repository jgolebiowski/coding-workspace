...
def loadClass(folder, minNumberImages=0):
...
def loadAndPickleDataSet(dataSetDirectory, minExamplesPerClass, force=False):
    ...
    for folder in dataSetFolders:
        ...
        if ((os.path.exists(classFileName)) and (not force)):
            ...
            dataSet = loadClass(folder, minExamplesPerClass)
